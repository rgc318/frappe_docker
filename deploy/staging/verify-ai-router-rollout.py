...
def _affinity_routes(map_path: str | Path) -> dict[str, str]:
    ...
    if routes.get("__default__") != "ai_affinity_missing":
        raise ValueError(
            "Release affinity map must contain __default__ ai_affinity_missing"
        )
    return routes
...
def evaluate(
    ...
    *,
    ...
    if (
        ...
        or stable_release == candidate_release
    ):
        violations.append(
            {
        ...
        violations.append(
            {
                ...
                "message": "Router returned unapproved release IDs: "
    ...
    tolerance = (
        max(0.03, 4 * math.sqrt(expected_ratio * (1 - expected_ratio) / total))
        if total
        else 1.0
    )
    ...
    unique = list(
        {(item["code"], item["message"]): item for item in violations}.values()
    )
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify persisted and observed AI rollout traffic."
    )
