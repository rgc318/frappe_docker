...
def render_map(entries: dict[str, str]) -> str:
    ...
    lines.extend(
        f"{bucket} ai_candidate" for bucket in range(100) if str(bucket) in entries
    )
    return "\n".join(lines) + "\n"
...
def _socket_command(container: str, command: str) -> str:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            result.stderr.strip() or "HAProxy runtime socket command failed"
        )
...
def _apply_runtime_map(
    container: str, runtime_map_path: str, entries: dict[str, str]
) -> None:
    ...
    if not match:
        raise RuntimeError(
            f"HAProxy did not create a map transaction: {prepare_output.strip()}"
        )
    ...
    try:
        ...
        raise
    verify_entries(
        parse_show_map(_socket_command(container, f"show map {map_id}")), entries
    )
...
def apply_rollout(
    *,
    ...
    stable_release_id = validate_release_id(
        stable_release_id, field="stable_release_id"
    )
    candidate_release_id = validate_release_id(
        candidate_release_id, field="candidate_release_id"
    )
    stable_pool_release_id = validate_release_id(
        stable_pool_release_id,
        field="stable_pool_release_id",
    )
    retired_release_id = validate_release_id(
        retired_release_id, field="retired_release_id"
    )
    ...
    try:
        ...
        try:
            _apply_runtime_map(
                container, runtime_affinity_map_path, old_affinity_entries
            )
            _apply_runtime_map(container, runtime_map_path, old_entries)
        except Exception as rollback_error:
            error = RuntimeError(
                f"{error}; runtime rollback also failed: {rollback_error}"
            )
        if old_state is not None:
            failed = {
                **old_state,
                "last_apply_error": str(error),
                "last_apply_failed_at": datetime.now(UTC)
                .isoformat()
                .replace("+00:00", "Z"),
            }
        else:
...
def parse_affinity_file_entries(content: str) -> dict[str, str]:
    ...
    if entries.get("__default__") != "ai_affinity_missing":
        raise ValueError(
            "Release affinity map must retain __default__ ai_affinity_missing"
        )
    return entries
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Persist and atomically apply an AI candidate traffic percentage."
    )
    ...
    parser.add_argument(
        "--final-status",
        choices=("active", "draining", "promoting", "completed"),
        default="active",
    )
    ...
    parser.add_argument(
        "--drain-action", choices=("promote_candidate", "retire_candidate")
    )
    ...
    if (
        args.final_status == "draining"
        and args.candidate_percent == 100
        and args.drain_action != "promote_candidate"
    ):
        parser.error(
            "100% candidate draining requires --drain-action promote_candidate"
        )
    if (
        args.final_status == "draining"
        and args.candidate_percent == 0
        and args.drain_action != "retire_candidate"
    ):
    ...
    if args.final_status == "promoting" and args.drain_action != "promote_candidate":
        parser.error(
            "--final-status promoting requires --drain-action promote_candidate"
        )
    ...
    if args.final_status == "draining" and not drain_deadline:
        ...
        drain_deadline = (
            (now + timedelta(seconds=args.drain_seconds))
            .isoformat()
            .replace("+00:00", "Z")
        )
    if args.final_status == "promoting" and (
        not drain_started_at or not drain_deadline
    ):
        parser.error(
            "--final-status promoting requires --drain-started-at and --drain-deadline"
        )
    state = apply_rollout(
