...
def evaluate(snapshot: dict[str, Any], *, expected_replicas: int) -> dict[str, Any]:
    ...
    for replica in replicas:
        ...
        if (
            replica.get("health_status") != "healthy"
            ...
            continue
        missing_fields = [
            field for field in IDENTITY_FIELDS if not readiness.get(field)
        ]
        if missing_fields:
            violations.append(
                {
    ...
    if ready_replicas:
        ...
        if router.get("ready") is not True:
            ...
            missing_fields = [
                field for field in IDENTITY_FIELDS if not router.get(field)
            ]
            if missing_fields:
                violations.append(
                    {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify a readiness-routed AI replica set."
    )
