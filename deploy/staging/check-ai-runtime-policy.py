...
def main() -> None:
    ...
    effective = [
        item for item in published if _is_effective(item.get("policy") or {}, now)
    ]
    if not effective:
        raise RuntimeError(
    ...
    if not tool_ready:
        raise RuntimeError(
            "Effective staging Runtime Policies have no tool-ready model"
        )
    if not vision_ready:
        raise RuntimeError(
            "Effective staging Runtime Policies have no vision-ready model"
        )
