...
def _require_label(labels: dict[str, str], name: str, expected: str) -> None:
    ...
    if actual != expected:
        raise ValueError(
            f"Image label {name} mismatch: expected {expected}, got {actual or '-'}"
        )
...
def _identity(image: dict[str, Any], *, repository: str, tag: str) -> dict[str, Any]:
    ...
    if f"{repository}:{tag}" not in image["repo_tags"]:
        raise ValueError(
            f"Image inspect does not contain expected tag {repository}:{tag}"
        )
    return {
...
def capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if canary.get("status") != "passed":
        raise ValueError(
            "Only a passed AI canary report can qualify a rollback release pair"
        )
    if str(canary.get("release_id") or "").strip() != release_id:
        raise ValueError(
            "AI canary report release_id does not match the release pair tag"
        )
    ...
    backend_revision = _require_revision(
        backend["labels"].get("org.rgc.myapp_revision"),
        name="Backend revision",
    )
    ai_revision = _require_revision(
        readiness.get("runtime_revision"), name="AI runtime revision"
    )
    if str(canary.get("backend_revision") or "").strip().lower() != backend_revision:
        raise ValueError(
            "AI canary Backend revision does not match the running Backend image"
        )
    for scenario in canary.get("scenarios") or []:
        ...
        if scenario_revision and str(scenario_revision).strip().lower() != ai_revision:
            raise ValueError(
                "AI canary mixed more than one Orchestrator runtime revision"
            )
    ...
    return {
        ...
        "backend": {
            **_identity(
                backend, repository=args.backend_repository, tag=args.backend_tag
            ),
...
def _verify_identity(
    *, expected: dict[str, Any], actual: dict[str, Any], role: str
) -> None:
    ...
    if expected_digests and not expected_digests.intersection(actual_digests):
        raise ValueError(
            f"{role} image digest does not match the qualified release pair"
        )
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _require_label(
        backend["labels"], "org.rgc.myapp_revision", manifest["backend"]["revision"]
    )
    _require_label(ai["labels"], "org.rgc.release_id", args.release_id)
    _require_label(
        ai["labels"],
        "org.opencontainers.image.revision",
        manifest["ai_orchestrator"]["revision"],
    )
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Capture or verify an immutable staging release pair."
    )
