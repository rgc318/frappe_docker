...
release_pair = _load_module(
    "staging_release_pair", "deploy/staging/staging-release-pair.py"
)
replica_set = _load_module(
    "staging_replica_set", "deploy/staging/verify-ai-replica-set.py"
)
...
rollout_verifier = _load_module(
    "staging_ai_rollout_verifier",
    "deploy/staging/verify-ai-router-rollout.py",
)
...
class ReleasePairTest(unittest.TestCase):
    def setUp(self):
        ...
        self.readiness_path = self._write(
            "ready.json",
            {
                "ready": True,
...
class ReplicaSetTest(unittest.TestCase):
    ...
    def test_replica_revision_drift_fails(self):
        ...
        self.assertIn(
            "AI_REPLICA_RUNTIME_DRIFT", {item["code"] for item in report["violations"]}
        )
...
class SloGateTest(unittest.TestCase):
    ...
    def test_contract_mismatch_is_always_critical(self):
        ...
        self.assertEqual(report["status"], "failed")
        self.assertIn(
            "AI_SLO_CONTRACT_MISMATCH", {item["code"] for item in report["violations"]}
        )
    ...
    def _run_gate(self, canary_report: dict, *, require_pass: bool = False):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            alert_state = json.loads(
                (report_root / "current-alert-state.json").read_text(encoding="utf-8")
            )
...
class RolloutMapTest(unittest.TestCase):
    def test_candidate_percentage_renders_exact_buckets(self):
        ...
        self.assertEqual(
            rollout.parse_file_entries(rollout.render_map(entries)), entries
        )
    ...
    def test_runtime_map_update_batches_transaction_commands(self, socket_command):
        ...
        rollout._apply_runtime_map(
            "router",
            "/var/lib/haproxy/ai-router/rollout.map",
            entries,
        )
    ...
    def test_failed_runtime_update_restores_persisted_map(self, apply_runtime_map):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                affinity_map_path.read_text(encoding="utf-8"), original_affinity
            )
            self.assertEqual(
                json.loads(state_path.read_text(encoding="utf-8"))["status"], "failed"
            )
    ...
    def test_failed_update_preserves_previous_active_state(self, apply_runtime_map):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            state_path = root / "rollout-state.json"
            map_path.write_text(
                rollout.render_map(rollout.desired_entries(5)), encoding="utf-8"
            )
            affinity_map_path.write_text(
                rollout.render_affinity_map(
                    rollout.affinity_entries("stable", "candidate"),
                ),
                encoding="utf-8",
            )
...
class RolloutVerifierTest(unittest.TestCase):
    ...
    def _sample(self, *, stable: int, candidate: int):
        return {
            ...
            "identities": [
                ...
                {
                    "identity": {"ready": True, "release_id": "candidate"},
                    "count": candidate,
                },
            ],
        }
    ...
    def test_expected_distribution_passes(self):
        report = rollout_verifier.evaluate(
            self._state(25),
            self._sample(stable=375, candidate=125),
    ...
    def test_candidate_auto_fallback_is_safe_but_blocks_progression(self):
        report = rollout_verifier.evaluate(
            self._state(25),
            self._sample(stable=500, candidate=0),
            candidate_buckets=set(range(25)),
            affinity_routes=self._affinity(),
        )
        self.assertEqual(report["status"], "failed")
    ...
    def test_promoting_state_allows_candidate_only_affinity(self):
        ...
        report = rollout_verifier.evaluate(
            state,
            self._sample(stable=0, candidate=500),
