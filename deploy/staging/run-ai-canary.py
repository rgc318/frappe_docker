...
def _classification(
    *, http_status: int | None, code: str | None, provider_code: str | None
) -> str:
    if provider_code:
        ...
        if provider_status.isdigit():
            return (
                "partial"
                if int(provider_status) in TRANSIENT_HTTP_STATUSES
                else "failed"
            )
...
def _run_readiness(
    *, base_url: str, timeout: int, expected_release: str
) -> dict[str, Any]:
...
def _run_scenario(
    ...
    *,
    ...
    try:
        ...
        if (
            name == "chat"
            and not str((response.get("message") or {}).get("content") or "").strip()
        ):
...
def main() -> int:
    ...
    base_url = os.environ.get(
        "AI_CANARY_BASE_URL", "http://ai-orchestrator:4010"
    ).rstrip("/")
    ...
    status = (
        ...
        else "partial" if "partial" in statuses else "passed"
    )
    report = {
        ...
        "backend_revision": os.environ.get("AI_CANARY_BACKEND_REVISION", "").strip()
        or None,
