...
def evaluate(
    ...
    *,
    ...
    for report in canary_reports:
        ...
        for scenario in report.get("scenarios") or []:
            if (
                ...
                or scenario.get("scenario") == "readiness"
            ):
                continue
            total += 1
    ...
    alerts = [{"severity": "critical", **item} for item in violations] + [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate AI canary/load reports against release SLOs."
    )
    ...
    return (
        ...
        else 10 if report["status"] == "warning" else 20
    )
