...
def validate(config):
    ...
    if "traefik" in services or "proxy" not in services:
        errors.append(
            "production requires the single HTTPS proxy, not the dashboard proxy"
        )
    for name in (*ERP_SERVICES, "ai-orchestrator"):
        ...
        if not digest and (
            not tag or tag.lower() in {"latest", "develop", "main", "master", "stable"}
        ):
        ...
        if any(
            v.get("type") == "bind" and "/apps" in v.get("target", "")
    ...
    password = (
        services.get("db", {}).get("environment", {}).get("MYSQL_ROOT_PASSWORD", "")
    )
    if len(password) < 16 or password.lower() in {
