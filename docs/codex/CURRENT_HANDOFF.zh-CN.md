...
- 审查既有 `32/32 offline + 64/64 live` 报告后发现，真实 Agent live 用例只覆盖商品工具，而且 runner 曾从 expected trajectory 缩窄候选工具，不能充分证明模型在完整白名单中自主选择。AI 提交 `517ae0e4ec8cb2a41c94694137424bea96368dd8` 将 Agent eval 默认候选改为完整 `TOOL_REGISTRY`；后续 `cf59aacdd64243643fe770cbcbbb6d8ec31d1438` 修复合成 Tool Sandbox 与真实 Backend Grounding 契约不一致的问题。仅显式权限子集用例才设置 `request.allowed_tools`，expected trajectory 继续只参与评分，不参与生产路由或 actual 生成。
- 核心数据集由 32 条扩展到 36 条，新增业务单据、现金流报表、商品加销售报表双工具、多轮对话中销售转采购报表四个 Agent critical 场景，覆盖 `search_products`、`query_business_documents`、`get_business_report` 三个白名单工具。独立多工具调用使用 `unordered_contains`，允许模型采用不同安全顺序；合成 Tool Sandbox 按模型实际选择的工具名匹配结果，不再要求 fixture FIFO 顺序。
- 修复 Grounding Guardrail 两个真实误判：单据编号中的数字不再作为金额/数量/计数声明扫描；状态 term 按最长重叠优先，`未完成/未付款` 不再同时识别为 `完成/付款`。新增反向回归确认工具只支持“已完成”时，模型声称“未完成”仍会被阻断。
- 本地最终验证通过：`uv sync --extra test --extra dev --frozen`、Ruff、pre-commit、宿主机 `140 passed + 9 subtests`、Docker test `140 tests OK`、runtime 镜像构建和 eval fixture 隔离、AI/父仓库相关 `diff --check`。绑定最终提交 `cf59aacd` 的正式 offline full gate 为 `36/36 PASS`；报告：`/tmp/myapp-ai-eval-offline-cf59aac.json`。
- Sol/Terra 上游短暂恢复后，首次绑定 `517ae0e` 的 full gate 为 `67/72 FAIL`。其中两个普通兼容查询用例的 expected 与 Prompt 的“界面已展示时不要重复明细”规则矛盾；合成 Sandbox 又把真实 Backend 明确为完整的经营报表错误标记为未知，并遗漏单据查询的 limit/完整性 scope。修复后五个失败场景分别在 Terra `4/4`、Sol `1/1` 重跑通过，没有降低工具、参数、授权、安全或 Grounding 阈值。
- `gpt-5.6-luna` 在同样完整三工具白名单下完成四个新增场景的单模型 live partial，结果 `4/4 PARTIAL_PASS`；业务单据、现金流、多工具和多轮上下文四项的工具选择、参数、调用预算、授权、轨迹和 Grounding 指标均为 `1.0`。其中多工具实际轨迹为模型自主生成的 `search_products(query="莫", search_fields=[item_name,nickname]) + get_business_report(report_type="sales")`，多轮场景正确沿用上一轮日期并将报表切换为 `purchase`。报告：`/tmp/myapp-ai-eval-live-expanded-luna-dirty.json`；带原文/轨迹的分段旁证为 `/tmp/myapp-ai-eval-live-order-alternatives-dirty.json`、`/tmp/myapp-ai-eval-live-remaining-luna-dirty.json`。这证明真实 Function Calling 链路达到预期，但 Luna 不是当前发布 Policy 的主模型/fallback，partial 不能替代正式 full gate。
- 当前默认 `opencode-deepseek-v4-flash` 最小聊天为 200，但四场景旁证出现 2 次 Provider 503、1 次严格工具参数 Schema 拒绝、1 次最终完整性 Grounding 拒绝；Qwen/GLM 在单据场景被严格参数 Schema 拒绝，MiniMax 在最终 Grounding 被拒绝。系统未为提高通过率降低任何 Schema 或安全阈值。
- 最终 AI `cf59aac` 已推送 `origin/develop`，远端 CI [30436235324](https://github.com/rgc318/myapp-ai/actions/runs/30436235324) 和 CodeQL [30436235401](https://github.com/rgc318/myapp-ai/actions/runs/30436235401) 均成功。绑定该 revision 的 Sol/Terra live full gate 为 `72/72 PASS`；offline/live 数据集 SHA 均为 `c2d72c935777b6291f0558f34525d32208a0f57371e73d7c808b314e305d1d0c`，Prompt manifest SHA 为 `cd819b70fd2e8b9f6a56a8caf8ef6c81da4d7fab8855890447f71fef5a9d48ce`，工具 manifest SHA 为 `3c84ef2cf105079d07c8eaf354c653ccfaf30b99eaeb37138c7321993fb4a2d0`，模型顺序为 `nvap-gpt-5.6-sol -> nvap-gpt-5.6-terra`，所有发布指标为 `1.0`。live 报告：`/tmp/myapp-ai-eval-live-cf59aac-sol-terra.json`。本地治理验证返回 `release_gate_eligible=true`、无错误和警告。父仓库 gitlink 已在本地指向新提交但尚未提交；当前没有部署或发布 Runtime Policy，下一步进入 staging 镜像构建、报告挂载、限范围 Policy 和真实 ERP E2E。
...
- 提交与推送已完成：AI `b0aa7cf + 14bb362`、Backend `6f2bdec`、父仓库 `74f32ab1 feat: complete verifiable agent release gate` 均已推送 `develop`；父仓库固定两个子模块指针，并提交 Runtime revision 注入、offline/live 报告路径和改造文档。远端 AI CI [30426645172](https://github.com/rgc318/myapp-ai/actions/runs/30426645172)、Security [30426645171](https://github.com/rgc318/myapp-ai/actions/runs/30426645171)、CodeQL [30426645091](https://github.com/rgc318/myapp-ai/actions/runs/30426645091)、Backend CI [30426645214](https://github.com/rgc318/myapp/actions/runs/30426645214) 和父仓库 Lint [30426809238](https://github.com/rgc318/frappe_docker/actions/runs/30426809238) 全部成功。当前下一步进入 A1.2；staging 镜像构建/部署、治理报告挂载、限测试范围 Runtime Policy 发布和真实 ERP E2E 仍需独立授权。当前没有部署或发布 Policy，staging 仍使用兼容路径。
...
- 根因不是会话列表接口或会话数量分页，而是 `@ant-design/x` 会话项默认固定高度无法承载标题、元数据和待复核草稿标记；会话较多时多行内容溢出并覆盖相邻项。部署环境此前会话较少，所以现象不明显。
- Web 会话项已改为自适应高度，标题和元数据保持单行省略，待复核草稿标记独立展示；桌面侧栏与移动端 Drawer 共用独立纵向滚动区域，并增加稳定滚动条占位和滚动边界控制，避免长列表撑开工作区或互相覆盖。
- Web 验证通过：`npm run tsc`、`npm run biome:lint`、Jest 37 套/245 项、AI 定向测试 18 项、`npm run build` 和 `git diff --check`；生产 CSS 已确认包含自适应高度与列表纵向滚动规则。相关 API 契约未修改，原有会话列表、详情和归档接口继续由现有测试覆盖。
- 代码已提交并推送：Web `64acf7a fix: prevent AI conversation list overlap`。设计文档同步补充会话侧栏滚动、会话项自适应高度和超长文本省略约束。
- 镜像构建 run `30207978260` 成功，发布 `ghcr.io/rgc318/myapp-web:staging-20260726-64acf7a`；Web 部署 run `30208595016` 成功，目标为 `vivy@192.168.31.229`、宿主端口 `30080`。
- 部署后直连验收：`myapp-web-staging` 使用上述不可变镜像并处于 `running / healthy`；`/healthz`、`/user/login`、`/api/method/ping` 均为 HTTP 200。ERP/AI 镜像未重建、未替换，服务器既有 `backups/`、`tmp/` 保持不动。

### 2026-07-26 AI 上下文生命周期增强（已提交、推送并部署 staging）
...
- 已提交并推送：Backend `de677b3 feat: add AI conversation context lifecycle`，Web `3fabcfe feat: add AI conversation context reset`；Web 布局修复初版为 `e4a78b9`，根据实际 ProComponents 版本源码确认 `tabs.cardProps` 不生效后，已由 `589378d fix: apply user detail tab spacing` 修正为直接给每个 Tab pane 绑定内容 class。AI Orchestrator `b374fc7 feat: add state-aware intent parsing` 已先推送，父仓库同步固定 Backend/AI gitlink。
- staging 发布已完成：ERP/AI 镜像构建 run `30199333914`，Web 镜像构建 run `30199384529`；ERP/AI 部署 run `30199593922`，Web 部署 run `30199593926`，均成功。目标服务器为 `vivy@192.168.31.229`，运行 Web `ghcr.io/rgc318/myapp-web:staging-20260726-589378d`、ERP `ghcr.io/rgc318/myapp-erpnext:staging-20260726-f1ee5b8`、AI `ghcr.io/rgc318/myapp-ai:staging-20260726-f1ee5b8`。
- 部署后检查：Web 容器 healthy，`/healthz`、`/user/login`、`/api/method/ping` 均 HTTP 200；ERP `staging.example.com` Ping HTTP 200；AI `/health` 为 `status=ok`，`intent_parse=erp-intent-v3`，向量检索和运行时治理均已配置。远端既有 `backups/`、`tmp/` 未跟踪文件保持不动。

### 2026-07-26 用户详情布局间距修复（已提交、推送并部署 staging）
...
- Backend `list_ai_conversations_v1` 新增当前用户范围内的服务端搜索，规范化空白并限制为 100 个字符，以字面子串匹配会话标题或消息正文，`%`、`_` 等字符不会被当作通配符；原有状态筛选和分页保持不变。列表同时返回每个会话的 `pending_draft_count` 与当前用户全局 `pending_draft_total`，两者只统计仍需复核的 `draft` 状态草稿。
- Backend 新增 POST `rename_ai_conversation_v1`。活跃或归档会话都只能由其当前 owner 重命名；名称规范化后必须为 1～120 个字符。重命名只更新标题和修改审计字段，不改变消息、公司、会话状态或 `last_message_at` 排序。
- Web 桌面侧栏与移动 Drawer 接入服务端标题/消息搜索，显示筛选结果总数、消息数、最近消息时间和每会话待复核草稿数；活跃与归档会话均提供重命名，归档操作仍只出现在活跃列表。顶部“我的草稿”使用当前用户全局待复核数量 Badge，并在放弃草稿或草稿离开 `draft` 状态后刷新。
- Web 使用按会话 ID 分区的浏览器内存保存 Sender 尚未发送文本，新会话工作区使用独立键；切换会话或活跃/归档列表时恢复对应文本，发送成功启动后清空当前会话文本，归档时删除该会话缓存。该状态仅保留在当前页面生命周期，不写入 Backend 或浏览器持久存储。
- Backend 验证通过：`test_ai_repository + test_ai_service + test_gateway_wrappers` 共 190 项、公共 API 模块导入冒烟和 `git diff --check`；仍有既有 `Failed to log error in db: AI Orchestrator 流式调用失败` 测试日志，退出码为 0。Backend 提交：`0204af8 feat: improve AI conversation management`。
- Web 验证通过：`npm run tsc`、`npm run biome:lint`、36 套/240 项 Jest、`npm run build`、`npm audit --omit=dev`（0 漏洞）和 `git diff --check`。提交钩子格式化后再次通过 TypeScript、Biome 和 36 套/240 项 Jest；Jest 仍保留既有 open-handle 提示，退出码为 0。Web 提交：`af0af1a feat: streamline AI conversation management`。
- 两个提交均未推送、未构建镜像、未部署。AI Orchestrator 工作树干净；Mobile 原有 5 个未提交文件继续保留且未触碰；父仓库 `.codex` 继续保持未跟踪。
- 下一步：staging 使用不同账号验证搜索 owner 隔离、标题与正文命中、特殊字符字面搜索、归档会话重命名、草稿数量刷新和多会话未发送文本切换。代码路线进入默认智能模式下的场景/固定模型高级设置、友好模型名称、按角色开放诊断能力，以及普通工作区水印和长文本可读性优化。

#### 未完成范围快照与接手顺序

- 已完成但仍待 staging 人工验收：P0 草稿闭环、版本冲突恢复、错误码分类恢复、草稿关键业务摘要、每条回答独立 Run、数据新鲜度与完整结果入口、长会话游标分页，以及本节会话搜索/重命名/草稿数量/未发送文本隔离。验收必须继续覆盖不同公司和角色、并发编辑、权限变化、历史失败 Run、100 条以上会话、归档只读和重复提交保护。
- 下一项代码任务：简化普通工作台高级能力。保持默认自动场景，将场景和固定模型收入高级设置；模型展示使用友好名称，调试/诊断能力按角色开放，不能向普通用户泄露内部 alias、Provider 或治理细节。
- 后续体验任务：普通工作区水印与长文本可读性、复制上下文新建会话，以及会话草稿内存状态是否需要可选的浏览器持久化；任何持久化方案都必须先评估共享终端和敏感业务文本风险。
- 治理后台缺口：总览与模型/策略/用量/向量/审计页面真正拆分，策略 Steps 编辑，模型检查批次任务化与历史趋势，审计和数据治理结果结构化。
- 工程门禁缺口：共享草稿编排 Hook/控制器、大型 AI 页面组件拆分、完整浏览器 E2E、键盘与屏幕阅读器验收、性能预算、治理查询缓存和剩余文档版本一致性检查。
- 当前仓库快照：父仓库提交本节后预计 `develop` ahead 8，仅 `.codex` 未跟踪；Backend `develop` ahead 5 且干净；Web `main` ahead 8 且干净；AI Orchestrator 干净；Mobile 仍有原有 5 个未提交文件，后续任务不得顺带提交或回退。
...
- 已完成但仍待 staging 人工验收：P0 草稿闭环、版本冲突恢复、错误码分类恢复、草稿关键业务摘要、每条回答独立 Run、数据新鲜度与完整结果入口。验收必须覆盖不同公司/角色、并发编辑、历史失败 Run、权限变化、未知截断、刷新失败和重复提交保护。
- 下一项代码任务：长会话分页、增量加载或窗口化。实现时必须保证消息级 Run、citation、反馈、失败恢复和草稿入口不因卸载或分页丢失，并避免加载历史消息覆盖当前 Sender 文本或正在运行的请求。
- 日常效率缺口：会话搜索、重命名、最近更新时间、待复核草稿 Badge、按会话保存未发送文本，以及复制上下文新建会话。
- 工作台简化缺口：默认智能模式、场景/固定模型高级设置、友好模型名称、按角色开放调试能力，以及普通工作区水印和长文本可读性优化。
- 治理后台缺口：总览与模型/策略/用量/向量/审计页面真正拆分，策略 Steps 编辑，模型检查批次任务化与历史趋势，审计和数据治理结果结构化。
- 工程门禁缺口：共享草稿编排 Hook/控制器、大型组件拆分、完整浏览器 E2E、键盘与屏幕阅读器验收、性能预算、治理查询缓存和剩余文档版本一致性检查。
- 当前仓库快照：父仓库提交本节后预计 `develop` ahead 6，仅 `.codex` 未跟踪；Backend `develop` ahead 3 且干净；Web `main` ahead 6 且干净；AI Orchestrator 干净；Mobile 仍有原有 5 个未提交文件，后续任务不得顺带提交或回退。
