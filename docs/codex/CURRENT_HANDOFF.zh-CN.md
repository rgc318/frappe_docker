...
- 首次 Deploy `34830586123` 在 GHCR token/manifest EOF，未切换容器；恢复旧标签并确认旧服务健康。用户明确要求重新尝试后，Deploy `34831986136` 又因 Docker Hub 拉取已缓存 HAProxy 镜像 EOF 而在切换前失败。随后只拉取已构建的两个 GHCR 候选镜像，ERP digest `sha256:451f31dd650a0b297901d912570518c177c798085d1bf191b8cabb33aa396a47`、AI digest `sha256:5eb295840dc610a0a7bae2bff96ebfa19c2271ba5d6376ea6fc3be2630dc0c1c`，使用 `PULL_POLICY=never` 受控切换，没有重建候选或访问 Docker Hub。
- 切换前完整备份 `sites/staging.example.com/private/backups/20260914_183004-staging_example_com-*`；migrate、enable-scheduler 和 `RUN_AI_STAGING_CANARY=1 check-staging.sh` PASS。Backend/Workers/Scheduler/Frappe Frontend 使用 `staging-20260914-dd7cd77`，AI 使用同标签且 runtime revision 仍为 `6700dccb...`；Web Deploy `34833612276` PASS，运行 `staging-web-20260914-a62b480`。三个目标容器健康/运行且 RestartCount=0，Scheduler active，HTTP 首页/Ping 200，AI canary passed，发布配对已 qualified；SLO 仍为样本量不足的 warning，不宣称达标。
- 对截图草稿 `AI-DRAFT-b0d130c02b6148e4aaa5c83a1ce7d35f` 执行只读重建并 rollback：`delete_price_error=False`、`legacy_patch_keys=[]`、`price_rows=4`。正式 Item Price 仍为 Retail 3/Bottle、Standard Selling 3/Bottle、Wholesale 52/Box、Standard Buying 50/Box。校验未保存或执行草稿，用户在页面点击一次“保存草稿”后才会把自愈结果持久化为新版本。
...
- 在上一节四类草稿动作契约贯通基础上，新增 resolved_scope：冻结已解析商品/订单目标及库存商品、仓库、调整方向；未解析值允许首次绑定，数量/金额正常编辑保留。库存构建器多行输入明确拒绝，不再只取 items[0]。尚未增加 Web 字段禁用交互，当前由后端错误明确提示重新发起请求。
- Backend 容器内动作安全、AI service、repository、Gateway 433 tests PASS；本地 Backend 已重启。真实 HTTP test_ai_draft_action_http 2 tests PASS（22 秒），gpt-5.6-luna / 百事可乐-3，另用百事可乐-2 验证改绑拒绝；同时验证删除拒绝、合法修改生成、契约落库和 operation 篡改拒绝。测试草稿已放弃、会话已归档，没有执行 ERP 商品写入。
- Web 沿用上一轮 tsc、Biome、58 suites / 386 tests PASS，本增量未修改 Web；AI 没有新增改动。Backend/Web 及父仓方案/交接增量尚未提交；前一阶段提交和 AI 推送状态见下节，未新增推送或线上部署。用户既有父仓改动继续保留。
- 下一步：完善 Web 已绑定范围提示，再补原文目标/字段证据及库存方向语义对照。resolved_scope 只能阻止后续漂移，不能证明首次模型提取正确；完整 keep/set/clear、澄清 UI、逐能力证据及任务恢复仍未完成。

## 2026-09-06 提交后继续：四类草稿动作契约贯通，新增改动待提交
