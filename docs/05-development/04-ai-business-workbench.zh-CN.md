...
商品查询与后续完善草稿通过服务端受控实体引用衔接。上一轮只有一个已解析商品，或最近商品结果集只有一个实体时，“这个商品、它、刚才查询到的商品”等指代可以继承该 `item_code`；多个候选、过期上下文或已清除上下文必须要求用户重新选择。查询卡片提供“完善此商品”明确动作，未绑定目标的草稿编辑器提供现有商品搜索选择器；正式执行前仍重新检查当前权限、公司和商品版本。

同一机制统一覆盖销售/采购订单、销售/采购发票、客户和供应商。订单查询返回唯一单据时，“这个订单、刚才那个订单”会形成精确单据查询；订单结果中的唯一往来单位会投影为 typed 客户或供应商，后续“给这个客户再开一张订单”“向这个供应商采购”可复用稳定 ID。多候选、混合客户/供应商、空结果、过期或已清除上下文都必须保持 `ambiguous / not_found`，不得回退旧结果猜测。草稿人工编辑后重新投影商品和往来单位；新建正式对象只有执行成功后才进入活动实体。
