...
|---|---|---|---|---|
| `item-square-v1` | 商品主图 | 1:1 裁剪、拖动、缩放、90°旋转 | 1600 × 1600 WebP，起始质量 82 | 最短边 300px |
| `avatar-square-v1` | 用户头像 | 1:1 裁剪、拖动、缩放、90°旋转 | 512 × 512 WebP，起始质量 85 | 最短边 128px |
...
- 后端会纠正 EXIF 方向、真实解码图片、居中兜底裁剪、LANCZOS 缩放并重新编码为 WebP。
...
- `src/components/ImageEditorUpload.tsx`
  - 统一文件选择、读取当前图片、编辑弹窗、拖动、缩放、旋转和输出。
...
- `components/item-image-field.tsx` 继续使用 `react-native-image-crop-picker` 提供原生 1:1 裁剪。
- 相册和相机使用同一 `1600 × 1600 / quality 0.82` 策略。
- Web fallback 使用 Expo `allowsEditing` 和 `aspect: [1, 1]`；最终一致性仍由后端保证。
...
- Web：`npm run tsc`、`npm run biome:lint`、相关 Jest 和全量 Jest。
- Backend：容器内运行图片处理、媒体服务和用户头像单元测试。
- Mobile：`npm run lint`，并至少在 Android/iOS 原生裁剪器和 Web fallback 各验证一次。
- HTTP smoke 应验证横图、竖图、透明 PNG、超大来源、伪造扩展名、低分辨率和已有图片重新裁剪。
